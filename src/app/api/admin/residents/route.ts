  ...
  try {
    ...
    let formatted = (residents || []).map((r: any) => {
      ...
      return {
        ...
        gender: r.gender,
        owner_name: r.organizations?.name || 'Unassigned',
        property_name: property?.name || 'Unassigned',
        room_number: room?.room_number || '-',
        bed_label: bed?.bed_label || '-',
        check_in_date: activeAssignment?.check_in_date || r.created_at?.split('T')[0] || null,
        monthly_rent_paise: activeAssignment?.monthly_rent_paise || 0,
        deposit_paise: activeAssignment?.monthly_rent_paise ? activeAssignment.monthly_rent_paise * 2 : 0,
