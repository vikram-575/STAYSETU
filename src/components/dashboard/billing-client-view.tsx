...
import { cn, formatDate, buildWhatsAppLink, buildSmsLink } from '@/lib/utils'
import {
  FileText, Plus, MessageCircle, Phone,
...
interface BillingClientViewProps {
  initialInvoices: any[]
  initialOutstandingResidents: any[]
  initialTab?: string
}

export function BillingClientView({
  initialInvoices = [],
  initialOutstandingResidents = [],
  initialTab = 'invoices',
  ...
  const totalBilledPaise = useMemo(() => {
    return initialInvoices.reduce((s, i) => s + (i.total_paise || 0), 0)
  }, [initialInvoices])

  const totalCollectedPaise = useMemo(() => {
    return initialInvoices.reduce((s, i) => s + (i.paid_paise || 0), 0)
  }, [initialInvoices])

  const totalOutstandingPaise = useMemo(() => {
    return initialInvoices.reduce((s, i) => s + Math.max(i.balance_paise || 0, 0), 0)
  }, [initialInvoices])

  const overdueInvoices = useMemo(() => {
    return initialInvoices.filter((i) => i.status === 'overdue' || (i.due_date < today && (i.balance_paise || 0) > 0))
  }, [initialInvoices, today])

  const totalOverduePaise = useMemo(() => {
    return overdueInvoices.reduce((s, i) => s + Math.max(i.balance_paise || 0, 0), 0)
  }, [overdueInvoices])

  const displayedInvoices = useMemo(() => {
    if (activeTab === 'overdue') return overdueInvoices
    return initialInvoices
  }, [activeTab, overdueInvoices, initialInvoices])
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-screen-2xl">
      ...
      {/* Filter Tabs with Instant 0ms Switching */}
      <div className="overflow-x-auto pb-1 scrollbar-none">
        <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-xl text-xs font-bold w-max">
          <button
            ...
            className={cn(
              'px-3.5 py-1.5 rounded-lg transition whitespace-nowrap cursor-pointer',
              activeTab === 'invoices' ? 'bg-white text-gray-900 shadow-xs font-black' : 'text-gray-600 hover:text-gray-900'
            )}
          >
          ...
          <button
            ...
            className={cn(
              'px-3.5 py-1.5 rounded-lg transition whitespace-nowrap cursor-pointer',
              activeTab === 'outstanding' ? 'bg-white text-orange-700 shadow-xs font-black' : 'text-gray-600 hover:text-gray-900'
            )}
          >
            Who Owes Money ({initialOutstandingResidents.length})
          </button>
          <button
            type="button"
            ...
            className={cn(
              'px-3.5 py-1.5 rounded-lg transition whitespace-nowrap cursor-pointer',
              activeTab === 'overdue' ? 'bg-white text-red-700 shadow-xs font-black' : 'text-gray-600 hover:text-gray-900'
            )}
          >
      ...
      {activeTab === 'outstanding' && (
        <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-200 p-3.5 sm:p-6 shadow-xs space-y-4">
          ...
          <div className="block md:hidden space-y-2.5">
            {initialOutstandingResidents && initialOutstandingResidents.length > 0 ? (
              initialOutstandingResidents.map((r) => {
