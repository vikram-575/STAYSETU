    ...
    <section className="relative overflow-hidden bg-gradient-to-b from-[#F7FAF7] via-white to-[#F7FAF7] pt-6 pb-12 sm:pt-10 sm:pb-16 lg:pt-16 lg:pb-24">
      ...
      <div className="mx-auto max-w-7xl px-3.5 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-center">
          <div className="inline-flex items-center gap-1.5 sm:gap-2 rounded-full border border-[#16A34A]/25 bg-[#DCFCE7]/60 px-3.5 py-1.5 text-xs font-semibold text-[#14532D] shadow-2xs backdrop-blur-xs">
        ...
        <div className="mx-auto mt-6 sm:mt-10 max-w-5xl">
          <div className="rounded-2xl border border-gray-200/90 bg-white p-3 sm:p-5 shadow-xl shadow-gray-200/60 ring-1 ring-black/5">
            {/* Property Type Tabs & Integrated Instant PG Option */}
            <div className="flex items-center justify-between gap-2 border-b border-gray-100 pb-2.5 sm:pb-3 overflow-x-auto no-scrollbar -mx-1 px-1 sm:mx-0 sm:px-0">
                  ...
                  return (
                    <button
                      ...
                      className={`inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-semibold transition-all shrink-0 whitespace-nowrap active:scale-95 ${
                        isActive
                          ? 'bg-[#14532D] text-white shadow-xs border border-[#14532D]'
                          : 'border border-gray-200/90 bg-gray-50/80 text-[#647067] hover:bg-gray-100 hover:border-gray-300 hover:text-[#17211B]'
                      }`}
                    >
                      <span>{tab.label}</span>
                    </button>
              ...
              {/* Seamlessly Integrated Instant PG Trigger (Issue 9 & Issue 14 Fix) */}
              <button
                type="button"
                onClick={() => {
                ...
                className="hidden sm:inline-flex items-center gap-1.5 rounded-xl border border-emerald-200/90 bg-emerald-50/80 px-3 py-1.5 sm:px-3.5 sm:py-2 text-xs font-bold text-emerald-800 hover:bg-emerald-100 transition shrink-0 cursor-pointer"
                title="Instant Verified PG Booking"
              >
                <Zap className="h-3.5 w-3.5 text-emerald-700 fill-amber-400" />
                <span>Instant PG</span>
              </button>
            </div>
            ...
            <form onSubmit={handleSearchSubmit} className="mt-3 sm:mt-4 grid grid-cols-1 gap-2.5 sm:gap-3 sm:grid-cols-2 lg:grid-cols-12">
              ...
              <div className="relative lg:col-span-4">
                <label className="block text-xs font-bold uppercase tracking-wider text-[#647067] mb-1">
                  Location or City
                </label>
                ...
                {showSuggestions && (
                  <div className="absolute top-full left-0 z-30 mt-1.5 w-full rounded-xl border border-gray-200 bg-white p-2 shadow-xl">
                    <div className="px-2 py-1 text-xs font-bold uppercase tracking-wider text-gray-400">
                      Popular Localities
                    </div>
          ...
          {/* Quick Search Chips (Issue 12 & Issue 20: Clean label alignment & max 5 chips with toggle) */}
          <div className="mt-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-[#647067]">Quick Filters:</span>
              {(hero?.quickChips || []).length > 5 && (
                <button
                  type="button"
                  onClick={() => setShowAllChips(!showAllChips)}
                  className="text-xs font-bold text-[#16A34A] hover:text-[#14532D] transition flex items-center gap-1 cursor-pointer"
                >
                  <span>{showAllChips ? 'Show Fewer' : `+${(hero?.quickChips || []).length - 5} More Filters`}</span>
                </button>
              )}
            </div>

            <div className="flex items-center flex-wrap gap-2">
              {((showAllChips ? hero?.quickChips : hero?.quickChips?.slice(0, 5)) || []).map((chip) => {
                ...
                return (
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-medium transition shrink-0 whitespace-nowrap active:scale-95 ${
                      isSelected
                        ? 'bg-[#16A34A] text-white shadow-xs border border-[#16A34A]'
